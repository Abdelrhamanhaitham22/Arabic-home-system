...
function Home({ t }: { t: Copy }) {
  return <div className="page home-page"><section className="hero-section"><div className="hero-copy"><p className="eyebrow"><span className="eyebrow-dot" />{t.eyebrow}</p><h1>{t.hero.split('\n').map((line, index) => <span key={line} className={index ? 'accent-line' : ''}>{line}<br /></span>)}</h1><p className="hero-body">{t.heroBody}</p><div className="hero-actions"><Link className="button primary" to="/register">{t.start}<span>↗</span></Link><Link className="button text-button" to="/results">{t.check}<span>→</span></Link></div></div><div className="hero-visual" aria-hidden="true"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="orbit orbit-three" /><div className="visual-core"><span>أ</span><small>مدار</small></div><div className="visual-note note-top">01 <span>learn</span></div><div className="visual-note note-bottom">result <span>→</span></div></div></section><section className="trust-line"><span className="line" /><p>{t.trust}</p><span className="line" /></section><section className="home-paths"><div className="path-lead"><span className="path-index">01</span><h2>{t.register}</h2><p>{t.registerBody}</p><Link to="/register" className="arrow-link">{t.start} <span>↗</span></Link></div><div className="path-lead secondary"><span className="path-index">02</span><h2>{t.results}</h2><p>{t.resultsBody}</p><Link to="/results" className="arrow-link">{t.check} <span>↗</span></Link></div></section></div>
}
